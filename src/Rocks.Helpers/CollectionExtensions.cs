...
namespace Rocks.Helpers
{
	public static class CollectionExtensions
	{
		...
		public static IEnumerable<TItem> SortById<TItem, TId> (this IEnumerable<TItem> items, IEnumerable<TId> ids, Func<TItem, TId> getId)
		{
			...
			foreach (var item in items.OrderBy (x => id_index[getId (x)]))
				yield return item;
		}
		...
		{
			return DistinctBy (items, keySelector, null);
		}
		...
		public static IEnumerable<T> DistinctBy<T, TKey> (this IEnumerable<T> items, Func<T, TKey> keySelector, IEqualityComparer<TKey> comparer)
		{
			if (items == null)
