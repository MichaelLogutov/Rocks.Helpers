...
namespace Rocks.Helpers
{
    public static class CollectionExtensions
    {
        ...
        ///     </para>
        /// </summary>
        public static async Task MergeIntoAsync<TItem, TKey>([CanBeNull] this IEnumerable<TItem> newItems,
                                                             [CanBeNull] IEnumerable<TItem> existedItems,
                                                             [NotNull] Func<TItem, TKey> key,
                                                             [CanBeNull] Func<TItem, CancellationToken, Task> insert,
                                                             [CanBeNull] Func<TItem, TItem, CancellationToken, Task> update,
                                                             [CanBeNull] Func<TItem, CancellationToken, Task> delete,
                                                             CancellationToken cancellationToken)
        {
            ...
            if (insert != null)
            {
                ...
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    await insert(item, cancellationToken).ConfigureAwait(false);
                }
            }
            ...
            if (update != null)
            {
                foreach (var item in comparison.SourceInBoth)
                {
                    ...
                    var item_key = key(item);
                    var dest_item = comparison.DestinationInBoth.First(x => key(x).Equals(item_key));
                    await update(item, dest_item, cancellationToken).ConfigureAwait(false);
                }
            }
            ...
            if (delete != null)
            {
                ...
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    await delete(item, cancellationToken).ConfigureAwait(false);
                }
            }
        }
    }
