	...
	public static class StringFormattingExtensions
	{
		                                        ...
		                                        string days = " дн")
		{
			...
			if (time.TotalSeconds < 1)
				result = time.Milliseconds + msec;
			else if (time.TotalMinutes < 1)
				result = time.Seconds + sec + " " + time.Milliseconds + msec;
			else if (time.TotalHours < 1)
				result = time.Minutes + min + " " + time.Seconds + sec;
			else if (time.TotalDays < 1)
				result = time.Hours + hours + " " + time.Minutes + min;
			else
				result = time.Days + days + " " + time.Hours + hours + " " + time.Minutes + min;
